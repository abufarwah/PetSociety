  ...
  searchSimilarPets() {
    ...
    .then(data => {
      ...
      if (data.matches && data.matches.length > 0) {
        this.searchResultMessage = `Comparison complete! Found ${data.matches.length} matching result(s).`;
        // map db response to frontend post structure
        this.posts = data.matches.map((m: any) => ({
          id: m.post.id,
          type: m.post.type,
          title: m.post.title,
          species: m.post.species,
          excerpt: m.post.description,
          location: m.post.location,
          dateText: m.post.dateText,
          image: m.post.imageUrl,
          phone: m.post.phone,
          confidence: Math.round(m.confidence * 100) + '%'
        }));
      } else {
         this.posts = [];
         this.searchResultMessage = 'No match found above 60% confidence.';
      }
      this.cdr.detectChanges();
    })
    .catch(error => {
      console.error('Error connecting to AI service:', error);
      this.searchResultMessage = 'Image uploaded but failed to connect to the backend AI service.';
      this.cdr.detectChanges();
    });
  }
